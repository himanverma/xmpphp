...
class XMPPHP_XMPP extends XMPPHP_XMLStream {
  ...
  /*  Our Custom handlers from here */
  
  /**
   * 
   * @param type $xml
   */
  protected function get_raw($xml){
      $this->event("getRaw", $xml);
  }
  
  protected function get_history_handler($xml){
      $response = [];
      foreach($xml->subs[0]->subs as $node){
          if($node->name != "chat")
              continue;
          $response[] = [
              "jid" => $node->attrs['with'],
              "time" => $node->attrs['start']
          ];
      }
      $this->event("history", $response);
  }
