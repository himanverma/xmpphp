...
class XMPPHP_XMLStream {
  ...
  public function connect($timeout = 30, $persistent = false, $sendinit = true) {
    ...
    do {
      ...
      try {
        ...
        $this->socket = @stream_socket_client("$conntype://{$this->host}:{$this->port}", $errno, $errstr, $timeout, $conflag,stream_context_create($options));
      } catch (Exception $e) {
  ...
  public function processUntil($event, $timeout=-1) {
    ...
    $updated = '';
    while (!$this->disconnected && $this->until_count[$event_key] < 1 && (time() - $start < $timeout || $timeout == -1)) {
      $this->__process();
    }
    ...
    if ($this->xml_depth == 1) {
      ...
      foreach ($this->xpathhandlers as $handler) {
        if (is_array($this->xmlobj) && array_key_exists(2, $this->xmlobj)) {
          ...
          if (($nstag[0] == null || $searchxml->ns == $nstag[0]) && ($nstag[1] == "*" || $nstag[1] == $searchxml->name)) {
      ...
      foreach ($this->nshandlers as $handler) {
        ...
        if ($searchxml !== null && $searchxml->name == $handler[0] && ($searchxml->ns == $handler[1] || (!$handler[1] && $searchxml->ns == $this->default_ns))) {
          if ($handler[3] === null)
            $handler[3] = $this;
